...
public class AlxLocationManager implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
     ...
     * @param mainActivity
     */
    public static void onCreateGPS(final Application context, AppCompatActivity mainActivity) {
        PermissionUtils.verifyStoragePermissions(mainActivity);
        sContext = context;
     ...
     * @param context
     */
    public static void restartGPS(Application context,MainActivity activity) {
        stopGPS();//先停止当前的GPS
        onCreateGPS(context, activity);//重启GPS
    }
    ...
    public boolean getCurrentLocation() {
        ...
        if (last_accuracy < MAX_deviation) {
            Logger.i("111111111"+latitude+"---"+longitude+"---"+speed);
    ...
    public void onLocationChanged(Location location) {
        ...
        if (location.getAccuracy() < MAX_deviation) {//精度如果太小就放弃
            Logger.i("22222222"+location.getLatitude()+"---"+location.getLongitude()+"---"+location.getSpeed());
    ...
    public static void recordLocation(Context context, double latitude, double longitude, float accuracy, float speed) {
        Logger.i("manager"+latitude+"---"+longitude+"---"+speed);
