...
public class AlxLocationService extends IntentService {
        ...
        if ((locationProvider != null) && (!locationProvider.equals("")) && (PROVIDER_ARRAY.contains(locationProvider))) {
            try {
                ...
                if (currentLocation != null) {
                    mNewLatitude = currentLocation.getLatitude();
                    mNewLongitude = currentLocation.getLongitude();
                    mAccuracy = currentLocation.getAccuracy();
                    mSpeed = currentLocation.getSpeed();
                    mMyLocation = new MyLocation();

                    mMyLocation.setLatitude(mNewLatitude);
                    mMyLocation.setLongitude(mNewLongitude);
                    mMyLocation.setAccuracy(mAccuracy);
                    mMyLocation.setSpeed(mSpeed);
                    mMyLocation.setUpdateTime(System.currentTimeMillis());
                    ...
                    if (!isWrongPosition(mNewLatitude, mNewLongitude)) isDestory = true;
                }
            } catch (Exception ex) {
    ...
    public interface GpsServiceListener {
        //        void getGpsInfo(double lat,double lng,float accuracy,float speed);
        void getGpsInfo(MyLocation location);
    }

    public static void setGpsInfoListener(GpsServiceListener listener) {
        if (mMyLocation != null) {
            listener.getGpsInfo(mMyLocation);
        }
    }
}
